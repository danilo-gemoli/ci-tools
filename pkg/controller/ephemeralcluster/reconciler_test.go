...
func TestCreateProwJob(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			req:        reconcile.Request{NamespacedName: types.NamespacedName{Namespace: "ns", Name: "ec"}},
			wantRes:    reconcile.Result{},
			wantErr:    errors.New("terminal error: validate and default presubmit: invalid presubmit job pull-ci-openshift-ci-tools-main-cluster-provisioning: failed to default namespace"),
		},
		{
